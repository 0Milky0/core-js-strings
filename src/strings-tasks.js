...
function isPalindrome(str) {
  const strNorm = str.toLowerCase().replace(/\W/g, '');
  const revStr = strNorm.split('').reverse().join('');
  if (revStr === strNorm) return true;
  return false;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i] === arr[i].toUpperCase()) {
      invert += arr[i].toLowerCase();
    } else {
      invert += arr[i].toUpperCase();
    }
  }
  return invert;
}
